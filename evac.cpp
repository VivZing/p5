...
Evac::Evac(City *citie, int numCitie, int numRoads) : numCities(numCitie), time(1)
{
  ...
  for(int x = 0; x < numCities; x++)
  {
    ...
    city[city_id].roadIDs = new int[city[city_id].roadCount];
    // totalRoads += city[city_id].roadCount;
    for(int y = 0; y < city[city_id].roadCount; y++)
    {
      city[city_id].roadIDs[y] = road_id = citie[x].roads[y].ID;
...
{
  int road_id = city[city_id].roadIDs[road_index];
...
int Evac::dfs(int destination_id, int people_remaining)
{
  ...
  if(city[destination_id].evacCity == false) // not an evacuated city
  {
...
void Evac::bfs()//BFS
{
  ...
  while(!bfs_queue->isEmpty())
  {
    ...
    {
      destinationID = roads[city[city_id].roadIDs[j]].destinationCityID;
      ...
      if(city[destinationID].evacCity )//if city is evaccity
      {
        ...
        city[city_id].roadIDs[j] = city[city_id].roadIDs[city[city_id].roadCount];//move to end
      }
    }
